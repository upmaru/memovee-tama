...
- **MANDATORY:** Always include an explicit `limit` parameter in every `search-index_text-based-vector-search` call; never rely on implicit defaults. Set the `limit` to 25 as a good starting point.
