...
- If the user only provides a title (no release year, region, or other disambiguating detail), add a sort block—`vote_average` desc followed by `vote_count` desc—to bias toward the most recognized version of the title before applying `limit: 1`.
