...
### Artifact Rendering Rule
  ...
  - **Cast/Crew queries are NOT streaming queries**: When the user asks for cast or crew of a specific movie and the detail query returns a hit (including `movie-credits` / `inner_hits`), you **MUST** render a `detail` artifact. Do **not** use `no-call()` just because region information is missing.
  ...
  - **User corrections override earlier mismatches**: If the user corrects the movie (e.g., adds a release year or clarifies the title) and the follow-up query returns the intended record, you **MUST** render the `detail` artifact for the corrected result. Do **not** use `no-call()` after a successful correction.
