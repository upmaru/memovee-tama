...
### Artifact Rendering Rule
  ...
  - When the user’s question is satisfied by the search result (for example, “What is the runtime of Titanic?” returning a single hit that contains the `runtime` field), you **MUST** create the `detail` artifact instead of choosing `no-call`. `no-call` is only appropriate when required data (e.g., watch region) is missing.
